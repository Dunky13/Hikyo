...
fixture_dir=$(mktemp -d "${TMPDIR:-/tmp}/hikyo-oci-guard.XXXXXX")
...
DOCKER_BIN="$fixture_dir/missing" "$(dirname "$0")/check-oci-unused.sh" registry.example/hikyo:0.1.0 >/dev/null
for fixture in found error; do
	if DOCKER_BIN="$fixture_dir/$fixture" \
		"$(dirname "$0")/check-oci-unused.sh" registry.example/hikyo:0.1.0 >/dev/null 2>&1; then
