...
// TestHumanOnlyRefusesMachinePrincipalsAfterTheGrantCheck pins the human-only
// rule (#68) at the chokepoint that enforces it.
//
// Three properties, and each of them is the rule rather than a restatement:
//
//   - the predicate turns on the identity's CLASS, so a machine is refused;
//   - a HUMAN identity is not, which is what makes the refusal about the
//     artifact class rather than about the operation;
//   - a class-less identity — local host authority (bootstrap, break-glass,
//     `hikyo admin`) — is exempt, because it is the one caller a human-only
//     rule must not lock out: it is how an instance with no human yet gets one.
//
// It is tested here rather than through the service layer because
// machineRefused is a pure predicate over the resolved identity, and reaching
// it from above would need a minted machine credential to assert something the
// registry row already states.
func TestHumanOnlyRefusesMachinePrincipalsAfterTheGrantCheck(t *testing.T) {
	if !HumanOnly(OpImportPresence) || !HumanOnly(OpValueImport) {
		t.Fatal("the import operations are not on the human-only list")
	}
	if HumanOnly(OpValueSet) {
		t.Fatal("`values set` is not a human-only verb; marking it one would break every workload")
	}
	...
	cases := []struct {
		...
		{"workload credential", domain.ClassWorkload, true},
		{"automation credential", domain.ClassAutomation, true},
		{"provisioning connection", domain.ClassProvisioning, true},
		{"human session", domain.ClassHuman, false},
		{"local host authority", "", false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			caller := Identity{Principal: "usr_x", Class: tc.class}
			if got := a.machineRefused(caller, OpImportPresence); got != tc.refused {
				t.Errorf("machineRefused = %v, want %v", got, tc.refused)
			}
			// The rule is per operation, never global: the same identity must
			// still reach the operations it is entitled to.
			if a.machineRefused(caller, OpValueSet) {
				t.Error("a non-human-only operation was refused for the caller's class")
			}
		})
	}
}
