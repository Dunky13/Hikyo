...
VALUES (sqlc.arg(id), sqlc.arg(chain_org_id), sqlc.arg(chain_project_id), sqlc.arg(path), sqlc.arg(created_at));
...
SELECT id, org_id, project_id, path, created_at FROM folders
WHERE org_id = sqlc.arg(chain_org_id) AND project_id = sqlc.arg(chain_project_id) ORDER BY path;

-- name: RenameFolder :execrows
UPDATE folders SET path = sqlc.arg(path)
WHERE org_id = sqlc.arg(chain_org_id) AND project_id = sqlc.arg(chain_project_id) AND id = sqlc.arg(id);

-- name: DeleteFolder :execrows
DELETE FROM folders WHERE org_id = sqlc.arg(chain_org_id) AND project_id = sqlc.arg(chain_project_id) AND id = sqlc.arg(id);
