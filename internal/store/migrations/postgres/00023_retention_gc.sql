...
ALTER TABLE orgs
    ADD COLUMN retention_mode TEXT NOT NULL DEFAULT 'keep-if-either'
        CHECK (retention_mode IN ('keep-if-either', 'unlimited')),
    ADD COLUMN retention_age_seconds BIGINT NOT NULL DEFAULT 7776000
        CHECK (retention_age_seconds > 0),
    ADD COLUMN retention_revision_count BIGINT NOT NULL DEFAULT 10
        CHECK (retention_revision_count > 0);

ALTER TABLE projects
    ADD COLUMN retention_revision_count BIGINT,
    ADD COLUMN retention_age_seconds BIGINT,
    ADD CONSTRAINT projects_retention_pair CHECK (
...
ALTER TABLE snapshots
    ...
    ADD COLUMN collected_policy TEXT NOT NULL DEFAULT '',
    ADD CONSTRAINT snapshots_collected_pair CHECK (
        (payload_present = TRUE AND collected_at IS NULL AND collected_policy = '')
        OR (payload_present = FALSE AND collected_at IS NOT NULL AND collected_policy <> '')
    );
...
CREATE TABLE retention_runtime (
    ...
    last_prune_success TIMESTAMPTZ
);
