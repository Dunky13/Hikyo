...
-- every pinned input.
--
-- Its own table rather than a column on `projects`: sqlite cannot add a NOT
-- NULL column without a default and cannot drop one afterwards, and `projects`
-- cannot take the table-rebuild treatment 00009 used on `environments` —
-- environments, folders and grants all reference it, so DROP TABLE under
-- foreign_keys=1 would take them with it. A separate table reaches the same
-- shape with no default anywhere and no rebuild.
--
