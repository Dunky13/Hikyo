...
-- to exactly one account. SQLite cannot add a UNIQUE column with ALTER, so the
-- column is added plain and the uniqueness is a partial index below.
ALTER TABLE accounts ADD COLUMN webauthn_user_handle BLOB;
