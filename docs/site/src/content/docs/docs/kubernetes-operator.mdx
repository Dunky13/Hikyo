...
4. A Hikyo operator enables the per-project machine-`reveal` opt-in. **This is
   not yet available in the public API; it is tracked by #17 and #58.**
5. Grant `reveal` to the workload principal. **This is also unavailable in the
   public API until #17 and #58 ship.**

Until steps 4–5 are available, a workload principal can deliver config values
and secret presence only. A `HikyoSecret` mapping that names any secret key is
therefore refused with `Delivery=False/UndeliveredSecrets`; it cannot deliver a
complete mapped Secret through the public API today.
