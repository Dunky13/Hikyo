...
5. Grant `reveal` to the workload principal. The next authenticated reconcile
   can deliver the complete mapped Secret.
...
| `Delivery=False` | `EnvFromSkip` | Warning: an invalid environment name may be skipped by `envFrom`; sync remains true. |
...
names and extra acknowledgements both refuse delivery. The list is sent on
every fetch so Hikyo records which acknowledgement was active.
...
Hikyo reports `Delivery=False/EnvFromSkip` as a warning while sync remains
successful.

## Beside the operator

For CI, use render-and-apply: run `hikyo values export ...` and feed the
rendered Secret directly to `kubectl apply -f -`. Never commit rendered values
to the GitOps repository. This is a `values export` operation under the normal
authorization formula; CI needs `reveal` explicitly to export secret values.
