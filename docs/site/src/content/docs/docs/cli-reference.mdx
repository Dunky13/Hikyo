...
window the CLI opens one from the terminal with an authenticator code where the
environment's window allows it, and exits `3` naming the browser path where it
does not. The four conditions hold under `--config-only` too.
