...
```text
hikyo adapter create --origin URL --env ENV --kind repository --owner OWNER --repo REPO --prefix PREFIX --keys ID,ID [--stdin|--value-file FILE]
hikyo adapter update ADAPTER --origin URL [--keep-remote] [--stdin|--value-file FILE]
hikyo adapter update ADAPTER --target TARGET --env ENV --kind KIND --owner OWNER [--repo REPO] --prefix PREFIX --keys ID,ID [--keep-remote]
hikyo adapter update ADAPTER --move MOVE --origin URL [--stdin|--value-file FILE]
hikyo adapter update ADAPTER --move MOVE --target TARGET --env ENV --kind KIND --owner OWNER [--repo REPO] --prefix PREFIX --keys ID,ID
...
hikyo adapter target add|list --adapter ADAPTER
...
[Forgejo deployment adapters](/docs/deployment-adapters/) for custody,
...
`run`, `render`, `sync`, `adopt`, and `definitions` are reserved but not
implemented in the current 0.x build. The binary refuses them explicitly.
