...
`HIKYO_TOKEN`; a stored human session is refused.
...
`render`, `sync`, `adopt`, and `definitions` remain reserved top-level stubs.
Compose delivery uses `compose render` and `compose sync`; `adopt` and
`definitions` are not implemented in the current 0.x build.
