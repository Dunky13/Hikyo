...
controlling terminal, an enumerated confirmation of the environment and key set,
and a live disclosure window hold. Without a live window it exits `3`; open one
with the reveal ceremony first. `--config-only` carries no secrets, so it needs
no window.
