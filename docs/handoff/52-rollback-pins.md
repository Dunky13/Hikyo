...
orchestrated brief; reviewed on Standards + Spec axes; one fix round.
...
- **The `reveal-history` gate binds at staging time**, before any superseded
  secret plaintext is decrypted into the caller's drafts — staging is where
  historical material first moves. Restore of the CURRENT revision needs no
  history gate.
- **The gate uses the stricter of written-time and current classification**
  (review fix F1, blocking): `snapshot_entries.classification` is sticky to
  the historical value, so reclassifying a key secret→config does not launder
  its historical plaintexts past the gate. Pinned by conformance scenario
  `restore_gate_uses_written_time_classification`.
...
`snapshots.payload_present` and `pending_changes.source`.
...
- Creating or reassigning to a NON-current revision authorizes the history
  gate (`reveal-history` seam, `pin.set-history`); the row records
  `history_authorized` and `schema_override` as facts about creation time.
- **Renewal re-checks exactly the recorded gates** and grandfathers the
  schema decision; a later publish cannot retroactively add a gate.
...
- **Pinned delivery**: `Delivery.Fetch` routes through a live pin for the
  ...
  (pin + publish grants, plus the recorded history gate) on EVERY fetch; a
