...
| No `-race` anywhere in CI. | **Fixed.** New `race` job (8 vCPU, both engines): `go list ./... \| grep -v /internal/isolation$ \| xargs go test -race -count=1`. Local: all packages pass under the detector with PostgreSQL; no data race found. The isolation suite under `-race` exceeds the per-package timeout (it is the serial probe-based E2E suite), so it runs race-instrumented in the new weekly `race-isolation` workflow (8 vCPU, `-timeout 90m`, `workflow_dispatch` for on-demand) — local race-instrumented run passed. |
| No fuzz targets despite hand-rolled binary parsers and an XML/SAML surface. | **Fixed.** 16 native Go fuzz targets: `internal/crypto` (`FuzzParseHeader` round-trip, `FuzzReadLP` bounds, `FuzzOpen` seal/open + single-byte-flip rejection, `FuzzReadRootKey`, `FuzzParseArtifact`), `internal/crypto/backup` (`FuzzExtractTo`), `internal/samlsp` (`FuzzParseXML`, `FuzzParseResponse`, `FuzzParseMetadata`), `internal/scimproto` (`FuzzParseFilter`, `FuzzParsePatch`, `FuzzDecodeUser`, `FuzzDecodeGroup`), `internal/importer` (`FuzzParseTemplate`, `FuzzParseManifest`, `FuzzParseValuesFile`). Each ran 30 s locally without a failing input; seeds run as normal tests inside `test`. New `fuzz` CI job discovers every `Fuzz*` target dynamically (`go test -list`) and runs each for 15 s, so a new target cannot be forgotten. |
| No govulncheck / gosec / CodeQL. | **govulncheck: fixed, and it found something.** Binary-mode scan of the built `hikyo` binary in the `test` job (seconds, scans what the linker kept; source mode over the ~1000-package graph takes minutes and gigabytes). On go1.26.2 it reported 16 reachable standard-library vulnerabilities (GO-2026-6218 … GO-2026-4918) — **toolchain pinned to go1.26.6** in `go.mod`, after which the scan is clean. **CodeQL: added** (`.github/workflows/codeql.yml`, `go` + `javascript-typescript`, push-to-main + weekly + manual; no `pull_request` trigger by design — this repo never runs PR-controlled YAML with elevated permissions). **gosec: deliberately skipped** — govulncheck + CodeQL + the custom `internal/lint` analyzers cover the ask; gosec's signal on this tree is overwhelmingly G104/G304-style noise. |
...
- `go test -race -count=1 <all but isolation>` with PostgreSQL: green, no race
  reports. `go test -race -count=1 -timeout 60m ./internal/isolation/`: green.
...
4. After the first `main` run: confirm `race` (~5 min on 8 vCPU expected) and
