...
- `go test -race -count=1 <all but isolation>` with PostgreSQL on go1.26.6: green,
  no race reports. `go test -race -count=1 -timeout 90m ./internal/isolation/`
  with PostgreSQL: green (≈25 min uncontended on an M-series laptop; 53 min
  while the machine was also fuzzing and running govulncheck — hence 90 m).
