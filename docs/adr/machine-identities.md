...
**Every operation that creates, replaces, or expands a working path from a machine credential to plaintext requires `manage-identities(project)` ∧ `reveal(E)` for *every environment reachable in the resulting post-state*, plus reauthentication** (see the amendment note above). Three distinct operations fall under this one formula, and naming only the first is the mistake this section exists to prevent:

1. **Minting** a credential — first issue or replacement.
2. **Creating an `oidc-federation` binding**, which opens a path from an external identity to this project's plaintext exactly as a token does.
3. **Any grant mutation that expands a service account's effective authority** — because authority lives entirely in the grants, so a grant change silently re-scopes **every credential already in circulation**, including ones held by people who are no longer in the room.

**The third is the one an implementer will miss.** A grant landing on a machine principal is not an ordinary grant: it is a scope widening of every live credential of that principal, delivered instantly and without anyone re-presenting anything. It therefore requires **both** #15's ordinary granting authorization **and** this formula, evaluated over the post-state, **in one transaction**. Where the two disagree, the stricter refuses. A grant mutation that would expand a machine principal's reach into an environment the actor cannot `reveal` is refused outright, regardless of `manage-members` scope.

**Narrowing is not widening.** Removing a grant, revoking a credential, deleting a binding and reducing scope stay under the plain capability — #15's symmetric limit, so incident response is never gated on disclosure rights.
...
This is #16's rule for human identities, and it is load-bearing on Kubernetes: a pattern rule such as *any ServiceAccount in namespace `prod`* hands an Envweave principal to anyone holding `create serviceaccount` in that namespace — a far wider group than cluster-admin.
...
- **Fetches are conditional on the change token** (#11). A caller presenting the current token is told it is current and receives **no plaintext**; only a fetch that actually delivers values is a disclosure. This is not merely a bandwidth optimization — it is what keeps per-key disclosure auditing (§ *Audit attribution*) proportionate to the rate values change rather than the rate workloads poll, and **authorization is still evaluated on the conditional path**, so a caller who has lost `read` learns nothing, per #15's unauthorized-is-indistinguishable-from-nonexistent rule.
...
**Federated bindings may be re-activated per binding**, because a binding holds no bearer value — there is nothing an attacker can have captured and nothing to redistribute. Re-activation is a re-validation, not a trust: #16's rule for human OIDC links applies for the same reason, since a restore can resurrect a binding removed precisely *because* that workload was compromised. **A token presented against a re-activated binding must be freshly issued** — its `iat` later than the epoch bump — so a token captured before the restore cannot be replayed into the re-established binding.
...
**Volume is controlled by not disclosing, not by not recording.** The naive fear — a 40-key environment on a 60-second resync writing ~58,000 disclosure rows a day — assumes every poll is a disclosure. It is not, and the revision ADR already supplies the mechanism that makes this true rather than aspirational:

- **Fetches are conditional on the change token.** A poll presenting the current token receives *no values*, and a response that carries no plaintext is **not a disclosure**: it emits a lightweight access record, never per-key disclosure events.
- Disclosure volume therefore tracks the **rate at which values actually change**, not the rate at which workloads poll. A stable environment polled every 60 seconds produces disclosure events on publish days and none in between.
- **Aggregation survives only where #16 already permits it** — non-disclosure access records and authentication-failure floods, *bounded by aggregation, never dropped silently*. It never applies to a disclosure event.

*Rejected: one aggregated event per fetch carrying an inline key-id list* — this ADR's earlier position, which claimed conformance with #15 while reversing its explicit cardinality (*"Every disclosed key emits its own audit event"*, and again in that ADR's propagation to this ticket, *"disclosure (one per key)"*). Inline enumeration preserves the key *set* but destroys per-key event identity, individual disclosure times and ordering, and collapsing repeats destroys them across occurrences too. The volume argument that motivated it evaporates once conditional fetch is used, and where volume genuinely remains, the locked remedy is the audit log's policy-based retention (#11), not a weaker event model. An ADR may amend an upstream decision explicitly; it may not claim to conform while contradicting one.
...
- **Permission model ([#15](https://github.com/Dunky13/envweave/issues/15))** — the two credential classes are implemented as an immutable `kind` with the normative allowlists enforced by the grant API; the mint formula is implemented and **tightened** by the amendment above to cover grant mutations on machine principals, which widen every credential already in circulation; **the per-key disclosure rule is honoured at its locked cardinality**, with volume addressed by conditional fetch rather than by a weaker event model; revocation is effective at next fetch; the pin authority re-check is carried on every pinned delivery; machines do not reauthenticate; no machine principal holds `manage-*`, `project-settings` or any instance capability.
...
- **Operations spec (fog)** — default credential **and binding** lifetime; the instance maximum-lifetime ceiling; the concurrent-credential cap per service account; expiry-notification thresholds and SMTP delivery policy; the **maximum accepted federated-token age and `exp - iat` span**; JWKS refresh interval, staleness bound and unknown-`kid` rate limit; the aggregation window for non-disclosure access records; audit retention policy for machine disclosure events (#11), which is now the volume lever; and the admission budget shared with #16's pre-authentication paths.
