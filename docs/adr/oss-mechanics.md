# Wenv OSS project mechanics (ADR, locked 2026-08-05)

> **Correction (2026-08-06, synthesis [#27](https://github.com/Dunky13/wenv/issues/27), per this ADR's own amendment procedure — [#33](https://github.com/Dunky13/wenv/issues/33) reopened, correction cross-model reviewed with the synthesis set, re-closed):** three passages in this document misquoted the license decision as Apache-2.0. The locked decision ([#9](https://github.com/Dunky13/wenv/issues/9), resolution of 2026-07-29) is **MPL 2.0** — it *replaced* the Apache-2.0 committed at repo init (LICENSE swapped in commit e1f32a5), precisely because Apache-2.0 has zero capture resistance. The committed LICENSE file and mvp-boundary.md O5 both already implement MPL 2.0. This is a scrivener's error corrected in place, not a license change; no decision moves. The no-CLA claim's width is restated under MPL 2.0: file-level copyleft keeps *existing* files open, but does not stop new proprietary code beside old open code — so the pledge's real enforcement remains governance, not law, exactly as argued.

Context: the license decision ([#9](https://github.com/Dunky13/wenv/issues/9)) fixes MPL 2.0 (replacing the Apache-2.0 committed at repo init), DCO-not-CLA as a principle, and the public "no /ee" commitment as a positioning asset; the threat model ([#8](https://github.com/Dunky13/wenv/issues/8)) fixes mandatory maintainer security review for crypto/auth/adapter/delivery contributions, protected branches, and maintainer 2FA; the architecture ADR ([#22](https://github.com/Dunky13/wenv/issues/22)) fixes the supply-chain trust model (offline cosign key-pair signing the checksum manifest and image digests, pinned trust root, fail-closed installers, digest-pinned chart, full-commit-SHA-pinned toolchains and CI actions, SBOM per release); the API/CLI ADR ([#25](https://github.com/Dunky13/wenv/issues/25)) fixes that the compatibility freeze fires at the first *stable* SemVer release and that prerelease tags freeze nothing; the deployment-adapter ADR ([#28](https://github.com/Dunky13/wenv/issues/28)) and K8s ADR ([#19](https://github.com/Dunky13/wenv/issues/19)) fix the only two extension points (in-tree adapters behind the Go seam; the ESO provider path post-freeze). What those ADRs delegated here is the human machinery around them: **where the project lives, how the repository is shaped, how contributions and disclosures arrive, how releases are cut and signed by an actual person, who governs, and how a future hosted service is structurally prevented from corrupting the open-source edition.** This ADR fixes those.

Granularity note: this ADR fixes process and structure, not artifact content. The spec document set's contents → synthesis ([#27](https://github.com/Dunky13/wenv/issues/27)); what is in the first release → MVP boundary ([#26](https://github.com/Dunky13/wenv/issues/26)); CI pipeline steps and exact pinned action SHAs → implementation under #22's rules. A delegation satisfied in letter but violating an intent stated here reopens this ADR.

## Canonical home — GitHub, in an organization, no public mirror

**The canonical repository is Wenv's GitHub organization repository.** The repository currently sits in a personal namespace (`Dunky13/wenv`); **transfer into a dedicated GitHub organization is a fixed implementation step of this ADR**, because a load-bearing enforcement claim depends on it: GitHub cannot enforce collaborator 2FA on a personal repository — only an organization can require 2FA of all members. GitHub redirects the old URL after transfer, so the move is cheap now and only gets more expensive.
...
```
cmd/wenv/        # multicall entry point (server/operator/migrate/client verbs, #22)
...
`internal/` is a statement, not a habit: Wenv is a product, not a library, and no Go API is a compatibility surface. The frozen surfaces are `/api/v1` and the CLI (#25) — nothing else. This keeps the SemVer promise (§ Releases) meaning exactly what #25 says it means, with no accidental third-party importers to break.
